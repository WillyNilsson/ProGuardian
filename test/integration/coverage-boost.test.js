...
describe('Integration Coverage Boost Tests', () => {
  ...
  describe('File security comprehensive tests', () => {
    ...
    it('should handle all secureWriteFile options', async () => {
      ...
      const nestedFile = path.join('nested', 'deep', 'file.txt')
    ...
    it('should handle all secureCopyFile options', async () => {
      ...
      const dest1 = 'dest1.txt'
      const dest2 = 'dest2.txt'
      ...
      await assert.rejects(() => secureCopyFile(source, dest1), ValidationError)
    ...
    it('should handle secureCreateDir options', async () => {
      const dir1 = 'dir1'
      const dir2 = 'dir2'
      const nestedDir = path.join('parent', 'child', 'grandchild')
    ...
    it('should handle all file operation errors', async () => {
      ...
      const readOnlyFile = 'readonly.txt'

      // Test read errors
      await assert.rejects(() => secureReadFile(nonExistent), PermissionError)
      ...
      if (process.platform !== 'win32') {
        ...
        await assert.rejects(() => secureReadFile(readOnlyFile), PermissionError)
      ...
      await assert.rejects(() => secureReadJSON(badJson), ValidationError)
    })
  })
